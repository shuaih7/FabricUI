...
def draw_bbox_image(img, boxes, labels, scores):
    font = cv2.FONT_HERSHEY_SIMPLEX
    color = [(255,0,0), (255,165,0)]
    
    for box, label, score in zip(boxes, labels, scores):
        xmin, ymin, xmax, ymax = box
        img = cv2.rectangle(img, (xmin,ymin), (xmax,ymax), color[label], 3)
        img = cv2.putText(img, str(round(score,3)), (xmin,ymin), fontFace=font, 
